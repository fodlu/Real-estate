...
const EditProperty = () => {
	...
	useEffect(() => {
		const fetchProperty = async () => {
			try {
				...
				setError("Failed to load the property details.");
				setLoading(false);
			}
		};
		fetchProperty();
	...
	const handleNewImageChange = (e) => {
		...
		if (existingImages + newImages.length + files.length > 10) {
			setError("Total number of images cannot exceed 10");
			return;
		}
	...
	const handleSubmit = async (e) => {
		...
		Object.keys(formData).forEach((key) => {
			if (key === "amenities") {
				data.append("ammenities", JSON.stringify(formData[key]));
