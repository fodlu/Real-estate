...
const SellerDashboard = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				console.error("Failed to load the dashboard data: ", error);
				setLoading(false);
			}
		};
		fetchData();
	...
	const handleDelete = async (id) => {
		if (!window.confirm("Are you sure you do not want to delete this listing?"))
			return;
	...
	// to update the status of a listing (make it sold or sale)
				...
				<div className={s.listingsHeader}>
					...
					<div className={s.searchWrapper}>
						...
						<input
							...
							value={searchTerm}
							onChange={(e) => setSearchTerm(e.target.values)}
								...
								renderActions={() => (
									<div className={s.propertyActions}>
										<button
											...
											{p.statusButton === "sold" ? "Available" : "Sold"}
										</button>
					...
					<div className={s.tipsList}>
						<div className={s.tipCardHighViews}>
							...
							<p className={s.tipTextHighViews}>
								Your listing is trending. Try adding video tours to increase
								interest
							</p>
						</div>
