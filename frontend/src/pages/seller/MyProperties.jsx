...
import { HiOutlineCheckCircle, HiOutlineLibrary, HiOutlinePencil, HiOutlinePencilAlt, HiOutlineTrash } from "react-icons/hi";
...
const MyProperties = () => {
	...
	// to fetch the property from the server side for the seller
	const fetchMyProperties = async () => {
		try {
			...
			const props =
				Array.isArray(res.data) ? res.data : res.data.properties || [];
			setProperties(props);
			setLoading(false);
		} catch {
					...
					{!Array.isArray(properties) || properties.length === 0 ?
						<div className={s.emptyCard}>
							<div className={s.emptyIconWrapper}>
								<HiOutlineLibrary size={40} color='#94q3b8' />
							</div>
										...
										<>
											<div className={s.actionContainer}>
												<div className={s.selectWrapper}>
													<select
														value={
															p.status === "sale" ? "avaialable" : p.status
														}
														onChange={(e) => {
