...
const AdminUsers = () => {
	...
	const handleDelete = async (id) => {
		if (
			!window.confirm(
				"Are you sure you want to delete this user? This action cannotbe undone.",
			)
		)
			return;
	...
	return (
		<>
			<div className={s.containerHeader}>
				<div>
					<h1 className={s.headerTitle}>Users Management</h1>
                                ...
                                <tr key={user._id} className={s.tableRow}>
                                    <td className={s.tdUserInfo}>
                                        <div className="flex items-center gap-4">
                                            <div className={s.userAvatar}>{user.name.charAt(0).toUpperCase()}</div>

                                            <div>
                                                <div className={s.userInfoName}>{user.name}</div>
                                                <div className={s.userInfoId}>ID: {user._id.slice(-8).toUpperCase()}</div>
                                            </div>
                                        </div>
                                    </td>
