...
const Properties = () => {
	...
	const handleCheckboxChange = (category, value) => {
		...
		fetchProperties(updatedFilters);
	}; //to update filter once toggled the checkbox
								...
								<div className={s.priceHeader}>
									...
									<span className={s.priceValue}>
										{filters.maxPrice >= 100000000 ?
											`#${(filters.maxPrice)}`
										:	`#${(filters.maxPrice)}`}
										{/* :	`#${(filters.maxPrice / 100000).toFixed(1)}`} */}
									</span>
								</div>
								<input
									type='range'
							...
							<div className={s.filterSection}>
								...
								<div className={s.checkboxGroup}>
									{furnishingOptions.map((option) => (
										<label className={s.checkboxLabel} key={option.value}>
											<input
												type='checkbox'
