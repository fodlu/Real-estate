...
const PropertyDetail = () => {
	...
	const handleInquirySubmit = async (e) => {
		...
		try {
			...
			setInquiryStatus({
				loading: false,
				success: false,
				error: `Failed to send inquiry: ${error}`,
			});
		}
	};
	...
	const handleChatStart = async () => {
		if (!user) return navigate("/login");
		if (user.role !== "buyer")
			return alert("Only a buyer can start a chat with seller");
		try {
	...
	const prevImage = () =>
		...
		); // open an image in a box and go to the next or previous ones
					...
					<div
						...
						{property.images.slice(1, 5).map((image, idx) => (
							<div
								...
								<img src={image} alt='image' className={s.galleryImage} />
					...
					<div className={s.sidebarColumn}>
						<div
							...
							<div className={s.priceCardValue}>
								{property.status?.toLowerCase() === "rent" ?
									`#${Number(property.price).toLocaleString("en-NGA")}`
								:	formattedPrice}
								{property.status?.toLowerCase() === "rent" && (
